...
from google.cloud.discoveryengine_v1.types import ImportDocumentsRequest
...
def upload_pdf():
    try:
        ...
        try:
            trigger_indexing_gcs_uri(gcs_uri)
            indexing_message = "Datei erfolgreich hochgeladen. Indexierung läuft..."
        except Exception as e:
            app.logger.warning(f"Indexierung konnte nicht getriggert werden: {e}")
...
def trigger_indexing_gcs_uri(gcs_uri: str) -> None:
    """
    Imports the uploaded PDF into the Discovery Engine datastore (incremental).
    """
    endpoint = discoveryengine_api_endpoint(DATA_STORE_LOCATION)
    client = discoveryengine_v1.DocumentServiceClient(
        client_options={"api_endpoint": endpoint}
    )

    parent = datastore_branch_parent(PROJECT_ID, DATA_STORE_LOCATION, DATA_STORE_ID)

    request = ImportDocumentsRequest(
        parent=parent,
        gcs_source=ImportDocumentsRequest.GcsSource(
            input_uris=[gcs_uri],
            data_schema="content",
        ),
    ...
    app.logger.info(f"Import started for {gcs_uri}. Operation: {op.operation.name}")
